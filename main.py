...
async def start_msg(message: types.Message):
	if message.chat.type == 'private':
		...
		await message.answer_photo(photo=photo, caption=f'<b>Welcome {message.from_user.get_mention()}. Left Shop</b>', reply_markup=await main_menu(user_id))

@vip.callback_query_handler(text='back', state='*')
async def back_msg(call: types.CallbackQuery):
	await call.message.edit_caption(caption=f'<b>Welcome {call.from_user.get_mention()}. Left Shop</b>', reply_markup=await main_menu(call.from_user.id))
...
async def information_msg(call: types.CallbackQuery):
	await call.message.edit_caption(caption='<b>🏝 Information Left Shop\n\n❔ Все вопросы по замене только с доказательствами. Наш маркет отвечает за качество товара.</b>', reply_markup=await information_menu())
